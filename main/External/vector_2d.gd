...
func draw_triangle(center := Vector2.ZERO, angle := 0.0, radius := 10.0, tri_color := DEFAULT_COLOR) -> void:
	var points := PackedVector2Array()
	var colors := PackedColorArray([tri_color])
	for i in range(3):
...
func draw_circle_outline(
	pos := Vector2.ZERO, radius := 30.0, cir_color := Color(), thickness := 1.0
) -> void:
	var points_array := PackedVector2Array()
	for i in range(CIRCLE_VERTEX_COUNT + 1):
		var angle := 2 * PI * i / CIRCLE_VERTEX_COUNT
		var point := pos + Vector2(cos(angle) * radius, sin(angle) * radius)
		points_array.append(point)
	draw_polyline(points_array, cir_color, thickness, true)
